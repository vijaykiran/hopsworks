...
public class DatasetController {
    ...
    public String createDatasetForDownload(Users user, Project project, String dataSetName,
            String datasetDescription, String publicId)
            throws IOException {
        //Parameter checking.
        if (user == null) {
            throw new NullPointerException(
        ...
        String dsPath = File.separator + Settings.DIR_ROOT + File.separator
                + project.getName();
        String pathToDatasetFolder = dsPath;
        dsPath = dsPath + File.separator + dataSetName;
        Inode parent = inodes.getProjectRoot(project.getName());
        Inode ds = inodes.findByParentAndName(parent, dataSetName);
    ...
    private boolean createFolderWithoutTemplate(String path, String username,
            FsPermission fsPermission) throws IOException {
        boolean success = false;
        Path location = new Path(path);
        DistributedFileSystemOps dfs;
